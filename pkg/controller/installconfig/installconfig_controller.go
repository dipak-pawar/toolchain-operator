...
func (r *ReconcileInstallConfig) EnsureCheSubscription(logger logr.Logger, installConfig *v1alpha1.InstallConfig) error {
	ns, err := r.createNamespaceForChe(logger, installConfig)
	if err != nil {
		return err
	}
...
func (r *ReconcileInstallConfig) createNamespaceForChe(logger logr.Logger, installConfig *v1alpha1.InstallConfig) (string, error) {
	...
	if err != nil && errors.IsNotFound(err) {
		logger.Info("Creating a Namespace Che", "Namespace", cheNamespace)
		return cheNamespace, r.client.Create(context.TODO(), che.Namespace(cheNamespace))
	}
...
func (r *ReconcileInstallConfig) createCheSubscription(logger logr.Logger, ns string, installConfig *v1alpha1.InstallConfig) error {
	...
	if err != nil && errors.IsNotFound(err) {
		logger.Info("Creating a new subscription che", "Subscription.Namespace", cheSub.Namespace, "Subscription.Name", cheSub.Name)
